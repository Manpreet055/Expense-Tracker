...
function downloadPdf() {
  ...
  pdfPage.innerHTML = `<h1>Monthly Report (${monthName})</h1>`;
  let contributionData = JSON.parse(
    localStorage.getItem("Contributions") || "[]"
  );
  let saveContributions = createPdf(contributionData, "Contributions");
  pdfPage.appendChild(saveContributions);
  let expenseData = JSON.parse(localStorage.getItem("Expenses") || "[]");
  let saveExpenses = createPdf(expenseData, "Expenses");
  pdfPage.appendChild(saveExpenses);
  /*global html2pdf*/
  html2pdf()
    .set({
